...
<c:forEach  var = "i" begin = "1" end = "${Screen.seats}">
<c:set var="flag" value="0">
</c:set>
	<c:forEach  var = "y" items= "${seatsbooked}">	
	
	<c:if test="${y==i }">
 	<c:out value = "${i}"/><input type="checkbox" ${i==y? 'checked':'' }  disabled="disabled" name="Seats"  value="<c:out value = "${i}"/>">
 	<c:set var="flag" value="1">
	</c:set>
 	</c:if>
 	</c:forEach>
 	<c:if test="${flag==0}">
 	<c:out value = "${i}"/><input type="checkbox"  name="Seats" value="<c:out value = "${i}"/>">
 	</c:if>
      </c:forEach>
</td>
</tr>
